...
function HistoryRow({ row }: { row: UserPick }) {
  const lang = useLang();
  const home = teamDisplayName(row.homeTeamCode, row.homeTeam, row.homeTeamEn, lang);
  const away = teamDisplayName(row.awayTeamCode, row.awayTeam, row.awayTeamEn, lang);
  ...
  return (
    <div className="py-2.5 px-1 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div dir="ltr" className="flex items-center gap-1.5 text-[13px] font-semibold truncate">
          <Flag code={row.homeTeamCode} />
          <bdi dir="ltr" className="truncate">{home}</bdi>
          <span className="opacity-40 px-0.5">×</span>
          <bdi dir="ltr" className="truncate">{away}</bdi>
