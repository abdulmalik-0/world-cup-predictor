...
import { savePick } from '../api/predictions';
...
export const MatchCard = memo(function MatchCard({ match, pick }: { match: Match; pick?: Prediction }) {
  ...
  const save = useMutation({
    mutationFn: () => savePick(match.id, home, away),
    onSuccess: () => {
    ...
    onError: (e: unknown) => {
      ...
      toast(/window|closed|قبل/i.test(msg) ? t('errWindowToast') : t('errSaveToast'), 'error');
    },
  });
  ...
  return (
    ...
    >
      ...
      <TeamMatchup
        ...
        homeAr={match.homeTeam}
        homeEn={match.homeTeamEn}
        awayCode={match.awayTeamCode}
        awayAr={match.awayTeam}
        awayEn={match.awayTeamEn}
        className="text-center mt-3.5 font-bold text-[14px]"
      />
